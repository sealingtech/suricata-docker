...
RUN yum -y update && \
	...
	cd /tmp/build && \
	curl -L -o boost_1_62_0.tar.gz https://sourceforge.net/projects/boost/files/boost/1.62.0/boost_1_62_0.tar.gz/download?use_mirror=pilotfiber && \
	tar xzf boost_1_62_0.tar.gz && \
	cd boost_1_62_0 && \
	./bootstrap.sh --prefix=/tmp/boost-1.62 --with-libraries=graph && \
	./b2 install && \
	cd /tmp/build && \
	git clone https://github.com/01org/hyperscan && \
	mkdir -p ./hyperscan/build && \
	cd hyperscan/build && \
	cmake -DCMAKE_INSTALL_PREFIX:PATH=/tmp/hyperscan -DBUILD_STATIC_AND_SHARED=1 -DBOOST_ROOT=/tmp/boost-1.62/ ../ && \
	make && \
	make install && \
	...
	cd /tmp/build && \
	curl -L -O https://www.openinfosecfoundation.org/download/suricata-3.2.tar.gz && \
	tar xzf suricata-3.2.tar.gz && \
	cd suricata-3.2 && \
